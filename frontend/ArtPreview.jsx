...
function Comp({ art, seg, allModels, mini }) {
  const p = seg.palette;
  ...
  return (
    <div style={{ position: "relative", height: "100%", minHeight: mini ? 90 : 150, width: "100%", flex: 1, borderRadius: 8, overflow: "hidden", background: bg, display: "flex", alignItems: "center", padding: mini ? "10px 13px" : "16px 22px", color: p.ink }}>
      {art.bg === "usage" && model ? (
      ...
      {art.bg === "hero" ? <div style={{ position: "absolute", right: "12%", top: "50%", transform: "translateY(-50%)", width: "40%", aspectRatio: "1", borderRadius: "50%", background: `radial-gradient(circle,${p.glow},transparent 62%)` }} /> : null}
      <div style={{ position: "relative", zIndex: 2, maxWidth: "60%", display: "flex", flexDirection: "column", gap: mini ? 4 : 8 }}>
        <span style={{ fontFamily: "Inter", fontSize: mini ? 7 : 10, fontWeight: 600, letterSpacing: ".22em", color: p.accent }}>{seg.eyebrow}</span>
        <span style={{ fontFamily: "Space Grotesk", fontWeight: 700, fontSize: mini ? 14 : 26, lineHeight: 1.02, letterSpacing: "-.02em", whiteSpace: "pre-line" }}>{seg.headline}</span>
        <span style={{ alignSelf: "flex-start", marginTop: mini ? 2 : 4, fontFamily: "Inter", fontWeight: 600, fontSize: mini ? 8 : 12, padding: mini ? "4px 9px" : "8px 15px", borderRadius: 9999, background: p.accent, color: "#06121f" }}>{CAMPAIGN.promo}</span>
      </div>
      <div style={{ position: "absolute", right: art.bg === "usage" ? "30%" : "11%", top: "50%", transform: "translateY(-50%)", width: mini ? 26 : 46, height: mini ? 56 : 100, borderRadius: "16% 16% 11% 11%", background: p.bottle, border: "1px solid rgba(255,255,255,.16)", boxShadow: "0 14px 22px rgba(0,0,0,.4)" }}>
...
function FoldPreview({ art, layout, seg, allModels }) {
  ...
  return (
    <div style={{ position: "relative", width: "100%", aspectRatio: "16/10", borderRadius: 12, overflow: "hidden", background: "#fff", border: "1px solid #EEF2F6" }}>
      <div style={{ height: `${art.fold}%`, padding: 10, display: "flex" }}>
        <BannerLayout layout={layout} gap={8} cell={(i) => <Comp key={i} art={art} seg={seg} allModels={allModels} mini={n > 1} />} />
      </div>
